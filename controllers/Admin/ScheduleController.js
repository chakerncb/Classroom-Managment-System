...
createSchedule = async (req, res) => {
    ...
    const classroomReservations = await connection.execute('SELECT * FROM schedule WHERE CODE_C = :CODE_C AND DAY_OF_WEEK = :DAY_OF_WEEK AND ((START_TIME <= TO_TIMESTAMP(:START_TIME, \'HH24:MI:SS\') AND END_TIME > TO_TIMESTAMP(:START_TIME, \'HH24:MI:SS\')) OR (START_TIME < TO_TIMESTAMP(:END_TIME, \'HH24:MI:SS\') AND END_TIME >= TO_TIMESTAMP(:END_TIME, \'HH24:MI:SS\')))', [code_c ,day_of_week, start_time, start_time, end_time, end_time]);
    if (classroomReservations.rows.length > 0) {
        res.json({ success: false, message: 'This classroom is already reserved in this time' });
        return;
    }
    ...
    const teacherAvailability = await connection.execute('SELECT * FROM schedule WHERE IDT = :IDT AND DAY_OF_WEEK = :DAY_OF_WEEK AND ((START_TIME <= TO_TIMESTAMP(:START_TIME, \'HH24:MI:SS\') AND END_TIME > TO_TIMESTAMP(:START_TIME, \'HH24:MI:SS\')) OR (START_TIME < TO_TIMESTAMP(:END_TIME, \'HH24:MI:SS\') AND END_TIME >= TO_TIMESTAMP(:END_TIME, \'HH24:MI:SS\')))', [idt, day_of_week, start_time, start_time, end_time, end_time]);
    if (teacherAvailability.rows.length > 0) {
        res.json({ success: false, message: 'This teacher is already busy in this time' });
        return;
    }
