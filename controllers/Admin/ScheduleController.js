...
createSchedule = async (req, res) => {
    const { day_of_week , idm , idtm , semester , level_name , code_c , start_time , end_time } = req.body;
    ...
    try {
        ...
        const result = await connection.execute('INSERT INTO schedule (SCHEDULE_ID , IDM , IDTM , LEVEL_NAME , CODE_C , START_TIME , END_TIME , DAY_OF_WEEK , SEMESTER) VALUES (:schedule_id , :IDM , :IDTM , :LEVEL_NAME , :CODE_C , TO_TIMESTAMP(:START_TIME, \'HH24:MI:SS\'), TO_TIMESTAMP(:END_TIME, \'HH24:MI:SS\'), :DAY_OF_WEEK , :SEMESTER)', [schedule_id , idm , idtm , level_name , code_c , start_time , end_time , day_of_week , semester]); 
    ...
    try {
        ...
        if (result.rows.length > 0) {
            ...
            for (let i = 0; i < result.rows.length; i++) {
                ...
                const [moduleResults, typeModuleResults] = await Promise.all([
